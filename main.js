...
function rollInitiative() {
  const p1Mod = gameState.players[1].battleRoles.reduce((sum, r) => sum + r.initiativeModifier, 0);
  const p2Mod = gameState.players[2].battleRoles.reduce((sum, r) => sum + r.initiativeModifier, 0);
  const p1Roll = Math.floor(Math.random()*100) + 1 + p1Mod;
  const p2Roll = Math.floor(Math.random()*100) + 1 + p2Mod;
...
function renderPlayerBattleRoles(playerId, container) {
  ...
  roles.forEach(role => {
    ...
    card.innerHTML = `
      ...
      ${role.shield > 0 ? `<div style="font-size:12px; color:#3498db;">🛡️ ${role.shield.toFixed(1)}心</div>` : ''}
      <div class="buff-list">${renderBuffs(role.buffs)}</div>
      ${role.skillCooldowns.some(cd=>cd>0) ? `<div style="font-size:10px; color:#666;">冷却：${role.skillCooldowns.map((cd,idx)=>cd>0?`技能${idx+1}:${cd}`:'').filter(Boolean).join(' ')}</div>` : ''}
    `;
    // 选目标状态
    ...
    // 选操作角色状态
    if (!gameState.selectingTarget && playerId === gameState.currentPlayer && role.currentHp>0 && role.canAttack) {
...
function renderSkillList() {
  ...
  gameState.selectedRole.skills.forEach((skill, idx) => {
    ...
    const card = document.createElement('div');
    card.className = `skill-card ${cd>0 ? 'disabled' : ''}`;
    card.innerHTML = `
      <div class="skill-name">${skill.name}</div>
      <div class="skill-desc">${skill.description}</div>
      ${cd>0 ? `<div class="skill-cooldown">冷却中：${cd}回合</div>` : ''}
    `;
    if (cd === 0) card.addEventListener('click', () => handleSkillSelect(skill, idx));
...
function handleTargetSelect(target, targetPlayerId) {
  ...
  if (skill.targetType === 'allEnemy') {
    ...
    targets = gameState.players[gameState.currentPlayer].battleRoles.filter(r => r.currentHp>0);
  } else {
    targets = [target];
  }
  // 命中判定
  ...
  targets.forEach(t => {
    ...
    if (damage > 0) {
      ...
      alert(`对${t.name}造成${damage.toFixed(1)}心伤害！`);
    } else if (damage < 0) {
      const heal = -damage;
      t.currentHp = Math.min(t.maxHp, t.currentHp + heal);
      alert(`为${t.name}恢复${heal.toFixed(1)}心血量！`);
    }
    // 处理Buff
    if (effect.buff) {
